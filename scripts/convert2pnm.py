...
for i in range(len(db_path)):
    if (
        db_path[i][-len(argvs[2]) :].lower() in input_outputFileFormatOpenCV
        and db_path[i][-len(argvs[2]) :].lower() == argvs[2]
    ):
        img_paths_for_opencv.append(db_path[i])
    elif (
        db_path[i][-len(argvs[2]) :].lower() in input_outputFileFormatPillow
        and db_path[i][-len(argvs[2]) :].lower() == argvs[2]
    ):
...
pdf_img_paths = [
    path for path in db_path if ((".pdf" in path.lower()) and (argvs[2] == ".pdf"))
]
...
if not (
    argvs[3] in input_outputFileFormatOpenCV
    or argvs[3] in input_outputFileFormatPillow
    or argvs[3] in outputFileFormatPillow
    or argvs[3] == ".pdf"
):
...
if not (
    argvs[2] in input_outputFileFormatOpenCV
    or argvs[2] in input_outputFileFormatPillow
    or argvs[2] == ".pdf"
):
...
if (
    argvs[2] in input_outputFileFormatOpenCV
    and argvs[3] in input_outputFileFormatOpenCV
):
...
if argvs[2] in input_outputFileFormatPillow and (
    argvs[3] in input_outputFileFormatPillow or argvs[3] in outputFileFormatPillow
):
...
if argvs[2] == ".pdf":
    for i in range(len(pdf_img_paths)):
        ...
        if (
            argvs[3] in input_outputFileFormatPillow
            or argvs[3] in outputFileFormatPillow
        ):
            image.save(
                "{0}{1}".format(
                    pdf_img_paths[i].split("/")[-1][: -len(argvs[2])], argvs[3]
                )
            )
        else:
            image = np.asarray(image)[:, :, ::-1]
